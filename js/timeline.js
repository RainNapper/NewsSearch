...
function timeline(domElement) {
    ...
    var timeline = {},   // The timeline
        ...
        bandGap = 15,    // Arbitray gap between to consecutive bands
    ...
    timeline.band = function (bandName, sizeFactor) {
        ...
        band.parts = [],
        band.instantWidth = 1; // arbitray value
        ...
        var items = band.g.selectAll("g")
            ...
            .style("fill", function (d) { 
                var redness = 25 * d.urgency;
                return '#'+(redness * 255 * 255).toString(16);
            })
    ...
    timeline.tooltips = function (bandName) {
        ...
        function dataClick (d) {
            console.log(d);
            $.get("html/GetStory.php?sid="+d.id, function(data) {
                $('#story_title').html(JSON.parse(data).HL.HT);
                $('#story_date').html(JSON.parse(data).HL.CT);
                $('#story_content').html(JSON.parse(data).HL.TE);
            });
        }
