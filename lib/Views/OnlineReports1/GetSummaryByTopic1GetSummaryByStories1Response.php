...
class Views_OnlineReports1_GetSummaryByTopic1GetSummaryByStories1Response extends Views_ResponseViewAbstract
{
	/**
	...
	public function getHTML()
	{
		...
		if (!isset($this->response->StoryMLResponse->STORYML->HL))
		{
	...
	private function getItemContent($item)
	{
		$retval = '<table width="100%" class="bordered"><thead>' .
					...
					'<tr><td>' . $item->TE . '</td></tr>' .
					'<tr><td>' . $this->getOriginsContent($item->SR) . '</td></tr>' .
	...
	private function getOriginsContent($origins)
	{
		$retval = '<table width="100%" class="bordered"><thead><b>Origins</b></thead>';
		foreach ($origins as $items)
		{
			if (is_array($items))
			{
				foreach ($items as $item)
				{
					$retval .= '<tr><td>' . $item->Type . '</td><td>' . $item->_ . '</td></tr>';
				}
			}
			else
			{
				$retval .= '<tr><td>' . $items->Type . '</td><td>' . $items->_ . '</td></tr>';
			}
		}
		$retval .= '</table>';
		return $retval;
